...
def Optimize(data_input,param):
    ...
    for i in range(int(param['ndays']/days)):
        ...
        if i==0:
            ...
            E_EV_start=param['EV_batt_cap']*param['EV_SOC_min']+data_input.E_EV_req[0]
            #SOC_start=0.6
            SOC_start=Batt.SOC_min
            ...
            #else:
            T_init=T_init_
            E_EV_start=E_EV_start_
            SOC_start=SOC_start_
        if data_input.index.dayofyear[0]+i==120: #WHY THIS CONSTRUCTION?
            toy=1
        ...
        for col in data_input_.keys():
            param.update({col:dict(enumerate(data_input_[col]))}) #TO CONVERT TO DICTIONARY TO GIVE TO PYOMO
        Set_declare=np.arange(-1,data_input_.shape[0])
        if i==0:
            print(retail_price_dict)
            
        param.update({'dayofyear':data_input.index.dayofyear[0]+i, #WHAT IS THIS UPDATE FUNCTION FOR?
                      'SOC_max':aux_SOC_max,
                      'toy':toy,
        ...
        if sys.platform=='win32':
            ...
            opt.options["mipgap"]=0.001
        #print("bf opt")
        results = opt.solve(instance)#,tee=True) #to see optimizing process
        #print("af opt")
        global_lock.release()
        #results.write(num=1) #to see optimizing process
        ...
        # Do something when the solution is optimal and feasible
            ...
            E_EV_start_=df_1.loc[df_1.index[-1],'E_EV']
            SOC_start_=df_1.loc[df_1.index[-1],'SOC']
            if param['aging']:
...
def save_results(df,aux_dict,param):
    ...
    try:
        ...
        filename_save=('../Output/df_%(name)s_%(Tech)s_%(App_comb)s_%(Cap)s_%(conf)s_%(house_type)s_%(batCapEV)s_%(EV_P)s_%(EV_use_profile)s_%(profile_row_number)s.csv'%{'name':param['name'],'Tech':param['Tech'],'App_comb':name_comb,'Cap':int(param['Capacity']),'conf':name_conf,'house_type':param['ht'],'batCapEV':param['EV_batt_cap'],'EV_P':param['EV_P_max'],'EV_use_profile':param['EV_use'],'profile_row_number':param['profile_row_number']})
