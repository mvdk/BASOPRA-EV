...
def Concrete_model(Data):
    ...
    m.SOC_start=en.Param(initialize=Data['SOC_start'])
    ...
    #EV
    m.EV_use=en.Param(initialize=Data['EV_use'])
    m.EV_batt_cap=en.Param(initialize=Data['EV_batt_cap'])#kWh
    m.EV_P_max=en.Param(initialize=Data['EV_P_max'])#kW
    m.EV_SOC_max=en.Param(initialize=Data['EV_SOC_max'])#%
    m.EV_SOC_min=en.Param(initialize=Data['EV_SOC_min'])#%
    m.EV_efficiency=en.Param(initialize=Data['EV_efficiency'])#%
    m.E_EV_start=en.Param(initialize=Data['E_EV_start'])#kWh

    
    
    m.E_EV_req=en.Param(m.Time,initialize=Data['E_EV_req'])#kWh
    #m.EV_P_max_home=en.Param(m.Time,initialize=Data['EV_home'])#fraction
    m.EV_home=en.Param(m.Time,initialize=Data['EV_home'])#fraction
    m.E_EV_trip=en.Param(m.Time,initialize=Data['E_EV_trip'])#kWh
    ...
    m.E_cons_without_backup=en.Var(m.Time,bounds=(0,None),initialize=0)
    ...
    m.SOC=en.Var(m.tm,bounds=(m.SOC_min,m.SOC_max),initialize=m.SOC_start)
    ...
    #EV
    m.E_PV_EV=en.Var(m.Time,bounds=(0,None),initialize=0)
    m.E_batt_EV=en.Var(m.Time,bounds=(0,None),initialize=0)
    m.E_grid_EV=en.Var(m.Time,bounds=(0,None),initialize=0)
    m.E_EV=en.Var(m.tm,bounds=(0,None),initialize=m.E_EV_start)
    m.E_EV_loss=en.Var(m.Time,bounds=(0,None),initialize=0)
    m.E_EV_charge=en.Var(m.Time,bounds=(0,None),initialize=0)
    m.E_EV_discharge=en.Var(m.Time,bounds=(0,None),initialize=0)
    ...
    m.Balance_hp_demand_PV_r=en.Constraint(m.Time,rule=Balance_hp_demand_rule_PV)
    m.Balance_hp_demand_batt_r=en.Constraint(m.Time,rule=Balance_hp_demand_rule_batt)
    m.Balance_hp_demand_grid=en.Constraint(m.Time,rule=Balance_hp_demand_rule_grid)
    ...
    m.Grid_cons_without_backup=en.Constraint(m.Time,rule=Grid_cons_without_backup_rule)
    ...
    m.EV_charging=en.Constraint(m.Time,rule=EV_charging_rule)
    m.EV_discharging=en.Constraint(m.Time,rule=EV_discharging_rule) # this one has some problem
    m.EV_charge_max=en.Constraint(m.Time,rule=EV_charge_max_rule)
    m.EV_energy=en.Constraint(m.Time,rule=EV_energy_rule)
    m.EV_SOC_max_r=en.Constraint(m.Time,rule=EV_SOC_max_rule)
    m.EV_SOC_min_r=en.Constraint(m.Time,rule=EV_SOC_min_rule)
    m.EV_loss_r=en.Constraint(m.Time,rule=EV_loss_rule)
...
def Balance_hp_demand_rule_PV(m,i):
    '''
    Description
    -------
    Make sure HP can't charge with PV when turned off
    '''
    if m.heating==False:
        return (m.E_PV_hp[i],0)
    else:
        return en.Constraint.Skip
    
def Balance_hp_demand_rule_batt(m,i):
    '''
    Description
    -------
    Make sure HP can't charge with batt when turned off
    '''
    if m.heating==False:
        return (m.E_batt_hp[i],0)
    else:
        return en.Constraint.Skip
    
def Balance_hp_demand_rule_grid(m,i):
    '''
    Description
    -------
    Make sure HP can't charge with batt when turned off
...
def EV_charging_rule(m,i):
    '''
    Description
    -------
    The EV can charge from the grid, the PV system, and the battery otherwise it can not charge
    
    '''
    return(m.E_EV_charge[i],(m.E_PV_EV[i] + m.E_batt_EV[i] + m.E_grid_EV[i]))

def EV_charge_max_rule(m,i):
    '''
    Description
    -------
    Per time step the EV can only charge up to its maximum power when it is at home
    '''
    return(m.E_EV_charge[i]<=m.EV_P_max*m.EV_home[i])
     
def EV_discharging_rule(m,i):
    '''
    Description
    -------
    the EV discharges energy during a trip
    '''
    return(m.E_EV_discharge[i],m.E_EV_trip[i])

def EV_energy_rule(m,t):
    '''
    Description
    -------
    The total energy in the EV is the sum of charging and discharging
    '''
    if t==0: # -1:
        return(m.E_EV[t],m.E_EV_start)
    else:
        return (m.E_EV[t],m.E_EV[t-1]+m.EV_efficiency*m.E_EV_charge[t]-m.E_EV_discharge[t])

def EV_SOC_max_rule(m,i):
    '''
    Description
    -------
    The EV can not charge more than it's maximum state of charge
    '''
    return(m.E_EV[i] <= m.EV_batt_cap*m.EV_SOC_max)

def EV_SOC_min_rule(m,i):
    '''
    Description
    -------
    The EV hast to charge enough to meet it's minimum requirements based on the minimum SOC and the energy required for upcoming trips
    '''
    return(m.E_EV[i] >= m.EV_batt_cap*m.EV_SOC_min+m.E_EV_req[i])

def EV_loss_rule(m,i):
    '''
    Description
    -------
    A certain amount of energy gets lost when it is charging
    '''
    return(m.E_EV_loss[i],(1-m.EV_efficiency)*m.E_EV_charge[i])
...
def E_dis_hp_rule(m,i):
    ...
    '''
    if m.heating==False:
        return (m.E_dis[i],m.E_batt_load[i]+m.E_loss_inv_batt[i])#+m.E_batt_EV[i])
    else:
        if m.DHW==False:
            return(m.E_dis[i],m.E_batt_hp[i]+m.E_batt_load[i]+m.E_batt_bu[i]+m.E_loss_inv_batt[i])#+m.E_batt_EV[i])
        else:
            return(m.E_dis[i],m.E_batt_hp[i]+m.E_batt_hpdhw[i]+m.E_batt_bu[i]+m.E_batt_budhw[i]+m.E_batt_load[i]+m.E_loss_inv_batt[i])#+m.E_batt_EV[i])
...
def Grid_cons_rule(m,i):
    ...
    '''
    if m.heating==False:
        return(m.E_cons[i],m.E_grid_batt[i]+m.E_grid_load[i]+m.E_loss_inv_grid[i]+m.E_grid_EV[i])
    else:
        if m.DHW==False:
            return(m.E_cons[i],m.E_grid_batt[i]+m.E_grid_load[i]+m.E_grid_hp[i]+m.E_grid_bu[i]+m.E_loss_inv_grid[i]+m.E_grid_EV[i])
        else:
            return(m.E_cons[i],m.E_grid_batt[i]+m.E_grid_load[i]+m.E_grid_hp[i]+m.E_grid_hpdhw[i]+m.E_grid_budhw[i]+m.E_loss_inv_grid[i]+m.E_grid_EV[i])

        
def Grid_cons_without_backup_rule(m,i):
    '''
    Description
    -------
    Balance of grid consumption, includes the electricity consumed by the battery, the loads, HP1, hpdhw, BU, budhw and losses in the inverter (when charging the battery from the grid).
    ...
    '''
    if m.heating==False:
        return(m.E_cons_without_backup[i],m.E_grid_batt[i]+m.E_grid_load[i]+m.E_loss_inv_grid[i]+m.E_grid_EV[i])
    else:
        if m.DHW==False:
            return(m.E_cons_without_backup[i],m.E_grid_batt[i]+m.E_grid_load[i]+m.E_grid_hp[i]+m.E_loss_inv_grid[i]+m.E_grid_EV[i])
        else:
            return(m.E_cons_without_backup[i],m.E_grid_batt[i]+m.E_grid_load[i]+m.E_grid_hp[i]+m.E_loss_inv_grid[i]+m.E_grid_EV[i])
...
def Balance_PV_rule(m,i):
    ...
    if m.heating==False:
        return (m.E_PV[i],m.E_PV_load[i]+m.E_PV_batt[i]+m.E_PV_grid[i]
            +m.E_loss_conv[i]+m.E_loss_inv_PV[i]+m.E_PV_curt[i]+m.E_PV_EV[i])
    else:
        if m.DHW==False:
            return (m.E_PV[i],m.E_PV_load[i]+m.E_PV_batt[i]+m.E_PV_grid[i]
            +m.E_PV_hp[i]+m.E_PV_bu[i]+m.E_loss_conv[i]+m.E_loss_inv_PV[i]+m.E_PV_curt[i]+m.E_PV_EV[i])
        else:
            return (m.E_PV[i],m.E_PV_load[i]+m.E_PV_batt[i]+m.E_PV_grid[i]
            +m.E_PV_hp[i]+m.E_PV_hpdhw[i]+m.E_PV_bu[i]+m.E_PV_budhw[i]+m.E_loss_conv[i]+m.E_loss_inv_PV[i]+m.E_PV_curt[i]+m.E_PV_EV[i])
...
def Conv_losses_rule(m,i):
    ...
    '''
    if m.heating==False:
           return(m.E_loss_conv[i],((m.E_PV_load[i]+m.E_PV_grid[i]+m.E_PV_EV[i])/(m.Inverter_eff)+m.E_PV_batt[i])*(1-m.Converter_eff))
    else:
        if m.DHW==False:
            return(m.E_loss_conv[i],((m.E_PV_load[i]+m.E_PV_grid[i]+m.E_PV_hp[i]+m.E_PV_bu[i]+m.E_PV_EV[i])/(m.Inverter_eff)+m.E_PV_batt[i])*(1-m.Converter_eff))
        else:
            return(m.E_loss_conv[i],((m.E_PV_load[i]+m.E_PV_grid[i]+m.E_PV_hp[i]+m.E_PV_hpdhw[i]+m.E_PV_bu[i]+m.E_PV_budhw[i]+m.E_PV_EV[i])/(m.Inverter_eff)+m.E_PV_batt[i])*(1-m.Converter_eff))
...
def Inv_losses_PV_rule(m,i):
    ...
    if m.heating==False:
        return(m.E_loss_inv_PV[i],(m.E_PV_grid[i]
           +m.E_PV_load[i]+m.E_PV_EV[i])*(1-m.Inverter_eff)/(m.Inverter_eff))
    else:
        if m.DHW==False:
            return(m.E_loss_inv_PV[i],(m.E_PV_grid[i]
           +m.E_PV_load[i]+m.E_PV_hp[i]+m.E_PV_bu[i]+m.E_PV_EV[i])*(1-m.Inverter_eff)/(m.Inverter_eff))
        else:
            return(m.E_loss_inv_PV[i],(m.E_PV_grid[i]
           +m.E_PV_load[i]+m.E_PV_hp[i]+m.E_PV_hpdhw[i]+m.E_PV_bu[i]+m.E_PV_budhw[i]+m.E_PV_EV[i])*(1-m.Inverter_eff)/(m.Inverter_eff))
...
def Inverter_rule(m,i):
    ...
    '''
    if m.heating==False:
        return(m.E_PV_grid[i]+m.E_dis[i]+m.E_PV_load[i]+m.E_loss_inv[i]+m.E_PV_EV[i]<=m.Inverter_power*m.dt)
    else:
        if m.DHW==False:
            return(m.E_PV_grid[i]+m.E_dis[i]+m.E_PV_load[i]+m.E_PV_hp[i]+m.E_PV_bu[i]+m.E_loss_inv[i]+m.E_PV_EV[i]<=m.Inverter_power*m.dt)
        else:
            return(m.E_PV_grid[i]+m.E_dis[i]+m.E_PV_load[i]+m.E_PV_hp[i]+m.E_PV_hpdhw[i]+m.E_PV_bu[i]+m.E_PV_budhw[i]+m.E_loss_inv[i]+m.E_PV_EV[i]<=m.Inverter_power*m.dt)
...
def Converter_rule(m,i):
    ...
    '''
    if m.heating==False:
        return(m.E_PV_grid[i]+m.E_PV_batt[i]+m.E_PV_load[i]+m.E_loss_conv[i]+m.E_PV_EV[i]<=m.Inverter_power*m.dt)
    else:
        if m.DHW==False:
            return(m.E_PV_grid[i]+m.E_PV_batt[i]+m.E_PV_load[i]+m.E_PV_hp[i]+m.E_PV_bu[i]+m.E_loss_conv[i]+m.E_PV_EV[i]<=m.Inverter_power*m.dt)
        else:
            return(m.E_PV_grid[i]+m.E_PV_batt[i]+m.E_PV_load[i]+m.E_PV_hp[i]+m.E_PV_hpdhw[i]+m.E_PV_bu[i]+m.E_PV_budhw[i]+m.E_loss_conv[i]+m.E_PV_EV[i]<=m.Inverter_power*m.dt)
...
def Obj_fcn(m):
    ...
    Description
    -------
    The bill is calculated in two parts, the energy related part is the retail price times the energy consumed from the grid minus the EV charging costs avoided by charging it with local PV power and minus the export price times the PV injection. If there is demand peak shaving (a capacity tariff is applied) the maximum power taken from the grid (in kW) is multiplied by the DAILY capacity tariff ($/kW per day).
    '''
    return(sum((m.retail_price[i]*m.E_cons[i])
               - (m.retail_price[i]*m.E_PV_EV[i])
               - (m.retail_price[i]*m.Efficiency*m.E_PV_batt[i])
